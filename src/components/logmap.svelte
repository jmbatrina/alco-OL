<script>
  ...
  for (let i = 0;i<dispensers.length; i+=1){
    ...
    //     if (markers[i].level == "High") tag = Active_High_Tag;
    //     else if (markers[i].level == "Medium") tag = Active_Medium_Tag;
    ...
    //     else tag = Active_Low_Tag;
    //   }
    //   L.marker(markers[i].xy, {icon: tag}).bindTooltip(markers[i].location, 
    //   {
    //     permanent: true, 
    //     direction: 'top',
    //     offset: [0,-30]
